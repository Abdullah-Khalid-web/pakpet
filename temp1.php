...
<div>
    ...
    <form action="" method="post" enctype="multipart/form-data">
        <div class="form-outline mb-4 w-50 m-auto">
            <label for="user_info" class="form-label"> ' . htmlspecialchars($user_fname) . '`s ' . htmlspecialchars($user_lname) . '<br>' 
            . htmlspecialchars($user_fname) . '`s ID : ' . htmlspecialchars($user_id) . '<br>' 
            . htmlspecialchars($user_fname) . '`s email : ' . htmlspecialchars($user_email) . '<br>'
            . htmlspecialchars($user_fname) . '`s Phone no : ' . htmlspecialchars($user_phoneno) . '</label>
        </div>
    
        <div class="form-outline mb-4 w-50 m-auto">
            <label for="user_fname" class="form-label">First Name : <span class="red">*</span></label>
            <input type="text" name="user_fname" id="user_fname" class="form-control" placeholder="First Name" value="' . htmlspecialchars($user_fname) . '"
                autocomplete="off" required="required">
        </div>

        <div class="form-outline mb-4 w-50 m-auto">
            <label for="user_lname" class="form-label">Last Name : <span class="red">*</span></label>
            <input type="text" name="user_lname" id="user_lname" class="form-control" placeholder="Last Name" value="' . htmlspecialchars($user_lname) . '"
                autocomplete="off" required="required">
        </div>
        
        <div class="form-outline mb-4 w-50 m-auto">
            <label for="user_address" class="form-label">Address : <span class="red">*</span></label>
            <input type="text" name="user_address" id="user_address" class="form-control" placeholder="Address" value="' . htmlspecialchars($user_address) . '"
            autocomplete="off" required="required">
        </div>
        
        <div class="form-outline mb-4 w-50 m-auto">
            <label for="user_phoneno" class="form-label">Phone No : <span class="red">*</span></label>
            <input type="text" name="user_phoneno" id="user_phoneno" class="form-control" placeholder="Phone No" value="' . htmlspecialchars($user_phoneno) . '"
            autocomplete="off" required="required">
        </div>
        
        <div class="form-outline mb-4 w-50 m-auto">
            <button type="submit" name="save_changes" class="btn w-10 mx-1 addtocart">Save Changes</button>
        </div>
    </form>
</div>';
            ...
            <a class='nav-link text-center' href='product_details.php?product_id=$product_id'>
                <div class='porduct-box'>
                    ...
                    <form action='delete_product.php' method='post' style='display:inline;'>
                        ...
                        <button type='button' class='btn mx-1 addtocart' onclick='confirmDelete(this)'>Remove</button>
                    </form>
...
function confirmDelete(button) {
    if (confirm("Are you sure you want to remove this product?")) button.form.submit();
}
</script>
